...
class GreenBitAdapter(BaseModelAdapter):
    """The model adapter for GreenBitAI's low-bit models"""
    ...
    def load_model(self, model_path: str, from_pretrained_kwargs: dict):
        try:
            ...
            if not cache_dir:
                cache_dir = os.getenv('HUGGINGFACE_CACHE', '~/.cache/huggingface/hub')
            ...
            model, tokenizer = load_llama_model(model_path, cache_dir=cache_dir, groupsize=groupsize,
                                                double_groupsize=32, bits=bits, half=True, v1=False,
                                                asym=False, kquant=True, dtype=torch.half, use_gbe=True, mpq_type=1)
        except ModuleNotFoundError as e:
